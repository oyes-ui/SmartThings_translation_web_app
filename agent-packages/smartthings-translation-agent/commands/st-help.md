...
3. **검수 포인트**: audit 6항목(① 문법/자연스러움 ② 의미 충실 ③ 용어집 준수 ④ 현지화 ⑤ 대소문자 ⑥ 포맷·BX), section coherence(title↔description 맥락 일치), 안전 규칙(원본 Excel 불변 / 수정 전 승인 / 크레딧 사전확인 / 시크릿 미노출).
4. **명령 목록**(크레딧 표시): `/st-setup`(0) `/st-prompt`(0) `/st-glossary`(0) `/st-inspect`(0) `/st-sections`(0) `/st-highlight`(0) `/st-textbook`(0) `/st-rag`(0~) `/st-ragdb`(0/빌드시 크레딧) `/st-edit`(0) `/st-translate`(LLM) `/st-audit`(LLM). 각 한 줄 설명.
