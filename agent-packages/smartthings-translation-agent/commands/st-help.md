...
4. **권장 검수 절차**: 아래 5단계를 짧게 안내한다.
   ...
   5. `/st-obsidian-report`: 언어별 최종 판단과 반영 상태를 리포트에 갱신한다.
   AI `Good`/`Needs Revision`은 모두 후보이며, 최종 판단은 실제 셀·source group·용어집·RAG·언어 규칙으로 한다.
5. **명령 선택 가이드**: 전체 재평가는 `/st-story-review`, 특정 AI 판정의 이유는 `/st-audit-explain`, 과거 표현 근거는 `/st-rag`, 사용자 승인 후 Excel 반영은 `/st-edit`이라고 한 줄로 구분한다.
